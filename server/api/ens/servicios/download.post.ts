...
import { sort_options, filter_options } from '@/types/server/ens/ens_servicios';
import { type type_filter_selection } from '@/types/client/filter_payload';

export default defineEventHandler( async (event) => {
  try{
    const userSessionId = event.context.user.id;
    await hasUserPermission(userSessionId, PermissionsList.ENSSERVICIOS_EXPORT);
    ...
    const fileColumns = [
      ...
      { key: 'name_es', header: 'Servicio', width: 40 },
