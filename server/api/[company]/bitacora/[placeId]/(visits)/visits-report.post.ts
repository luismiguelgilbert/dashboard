export default defineEventHandler(async (event) => {
  try {
    ...
    const { data: payload, error } = await readValidatedBody(event, bitacora_visits_report_query_schema.safeParse);
    ...
    const userDataQuery = await serverDB.prepare(`
      ...
      ,a.visited_name
      ,a.visited_area
      ,initcap(c.name_es) as reason_name
      ,a.vehicle_name
      ,a.vehicle_plate
      ,a.comments_in
      ,a.comments_out
      ...
      and a.visit_start >= to_timestamp('${payload.start}', 'YYYY-MM-DD HH24:MI:SS')
      and a.visit_start <= to_timestamp('${payload.end}', 'YYYY-MM-DD HH24:MI:SS')
