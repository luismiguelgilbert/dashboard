export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    if (payload.is_new) {
      hasPermissions(event, [PermissionsList.BITACORA_RIDES_REASONS_CREATE])
    } else {
      hasPermissions(event, [PermissionsList.BITACORA_RIDES_REASONS_EDIT]);
    }

    await serverDB.exec('BEGIN');

    // Upsert data
    await serverDB.sql`insert into bita_rides_reasons 
