export default defineEventHandler(async (event) => {
  try {
    await hasPermissions(event, [PermissionsList.BITACORA_RIDES_REASONS_READ]);
    ...
    return (recordDataQuery.rows && recordDataQuery.rows[0])
      ? bitacora_rides_reasons_schema.parse(recordDataQuery.rows[0])
      : bitacora_rides_reasons_schema.parse({ id: payload.id, is_new: true })
  } catch (err) {
