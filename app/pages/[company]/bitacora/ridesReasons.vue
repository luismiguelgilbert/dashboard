...
import type { bitacora_rides_reasons } from '@@/shared/types/bitacora_rides_reasons';
...
const openNew = async () => {
  ...
  await navigateTo({ name: 'company-bitacora-ridesReasons-id', params: { id: newUniqueId }, query: { ...useRoute().query } })
};

const downloadFile = async () => {
  try {
    const response: Blob = await $fetch(`/api/${userCompany.value?.id}/bitacora/ridesReasons-download`, {
      method: 'POST',
      body: {
        ...queryPayload.value,
        page: undefined,
    ...
    link.href = url;
    link.setAttribute('download', 'Motivos-Viaje.xlsx');
...
onBeforeMount(() => {
  try {
    if (Object.keys(currentRoute.value.query).length) {
      const parsedQuery = bitacora_rides_reasons_query_schema.parse(currentRoute.value.query);
      push({ query: parsedQuery, replace: true });
      queryPayload.value = parsedQuery;
    } else {
      const parsedQuery = bitacora_rides_reasons_query_schema.parse(queryPayload.value);
      push({ query: parsedQuery, replace: true });
    }
  } catch {
    push({ query: {} });
...
<template>
  <UDashboardPanel
    ...
    :max-size="35"
    resizable>
    <UDashboardNavbar title="Motivos de Viaje">
      <template #leading>
      ...
      <template #right>
        <UiListToolbar
          ...
          :sort-items="bitacora_rides_reasons_sort_enum_client"
    ...
    <ClientOnly>
      <RidesReasonsList @row-clicked="async (row: bitacora_rides_reasons) => await navigateTo({ name: 'company-bitacora-ridesReasons-id', params: { id: row.id }, query: { ...useRoute().query } })" />
    </ClientOnly>
  </UDashboardPanel>
  ...
  <div
    ...
    <UIcon name="i-lucide-list-check" class="size-32 text-dimmed" />
  </div>
  ...
  <USlideover
    ...
    :open="true"
    @update:open="navigateTo({ name: 'bitacora-rides-reasons', query: { ...useRoute().query } })">
    <template #content>
      <ClientOnly>
        <NuxtPage />
      </ClientOnly>
