...
const openNew = async () => {
  ...
  await navigateTo({ name: 'company-bitacora-cars-id', params: { id: newUniqueId }, query: { ...useRoute().query } })
};

const downloadFile = async () => {
  try {
    const response: Blob = await $fetch(`/api/${userCompany.value?.id}/bitacora/cars-download`, {
      method: 'POST',
      body: {
        ...queryPayload.value,
        page: undefined,
...
onBeforeMount(() => {
  try {
    if (Object.keys(currentRoute.value.query).length) {
      const parsedQuery = bitacora_cars_query_schema.parse(currentRoute.value.query);
      push({ query: parsedQuery, replace: true });
      queryPayload.value = parsedQuery;
    } else {
      const parsedQuery = bitacora_cars_query_schema.parse(queryPayload.value);
      push({ query: parsedQuery, replace: true });
    }
  } catch {
    push({ query: {} });
...
<template>
  <UDashboardPanel
    ...
    <UDashboardNavbar title="Vehículos">
      ...
      <template #right>
        <UiListToolbar
          ...
          :sort-items="bitacora_cars_sort_enum_client"
    ...
    <ClientOnly>
      <CarsList @row-clicked="async (row: bitacora_cars) => await navigateTo({ name: 'company-bitacora-cars-id', params: { id: row.id }, query: { ...useRoute().query } })" />
    </ClientOnly>
  </UDashboardPanel>
  ...
  <USlideover
    ...
    :open="true"
    @update:open="navigateTo({ name: 'company-bitacora-cars', query: { ...useRoute().query } })">
    <template #content>
      <ClientOnly>
        <NuxtPage />
      </ClientOnly>
