...
onBeforeMount(() => {
  try {
    if (Object.keys(currentRoute.value.query).length) {
      const parsedQuery = bitacora_reasons_query_schema.parse(currentRoute.value.query);
      push({ query: parsedQuery, replace: true });
      queryPayload.value = parsedQuery;
    } else {
      const parsedQuery = bitacora_reasons_query_schema.parse(queryPayload.value);
      push({ query: parsedQuery, replace: true });
    }
  } catch {
    push({ query: {} });
...
<template>
  <UDashboardPanel
    ...
    <UDashboardNavbar title="Motivos de Visita">
      ...
      <template #right>
        <UiListToolbar
          ...
          :sort-items="bitacora_reasons_sort_enum_client"
    ...
    <ClientOnly>
      <ReasonsList @row-clicked="async (row: bitacora_reasons) => await navigateTo({ name: 'company-bitacora-reasons-id', params: { id: row.id }, query: { ...useRoute().query } })" />
    </ClientOnly>
  </UDashboardPanel>
  ...
  <USlideover
    ...
    :open="true"
    @update:open="navigateTo({ name: 'bitacora-reasons', query: { ...useRoute().query } })">
    <template #content>
      <ClientOnly>
        <NuxtPage />
      </ClientOnly>
