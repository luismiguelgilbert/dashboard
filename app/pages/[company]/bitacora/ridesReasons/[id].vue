...
const { dataRecord, dataRecordFetching, dataRecordUpdate, dataRecordUpdating } = useBitacoraRidesReasonsQueries();

const saveForm = async () => {
  try {
    if (selectedRowData.value) {
      selectedRowData.value.is_saving = true;
      const { error } = bitacora_rides_reasons_schema.safeParse(selectedRowData.value);
...
<template>
  <div class="wrapper">
    ...
    <main v-if="!dataRecordFetching && selectedRowData">
      <RidesReasonFormContentBasic :vertical="isMobile" :disable="dataRecordFetching || dataRecordUpdating" />
    </main>
  </div>
</template>
