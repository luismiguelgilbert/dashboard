...
const { mutateAsync, isPending } = useMutation({
  mutationFn: () => $fetch(`/api/${userCompany.value?.id}/bitacora/rideReason-upsert`, { method: 'POST', body: selectedRowData.value }),
  onSuccess: async () => {
    if (selectedRowData.value?.is_new) {
      ...
      queryClient.setQueriesData({ queryKey: [computedQueryKey.value] }, (cacheData: bitacora_rides_reasons_query_cache | undefined) => {
        cacheData?.pages.forEach((page) => {
          const recordIndex = page.findIndex(reason => reason.id === selectedRowData.value?.id);
          if (page[recordIndex]) {
            page[recordIndex] = {
              ...page[recordIndex],
...
const saveForm = async () => {
  try {
    if (selectedRowData.value) {
      ...
      const { error } = bitacora_reasons_schema.safeParse(selectedRowData.value);
...
<template>
  <div class="wrapper">
    <UDashboardNavbar
      ...
      <template #leading>
        <UButton
          ...
          @click="navigateTo({ name: 'company-bitacora-ridesReasons', query: { ...useRoute().query } })" />
      </template>
      <template #right>
        <UButton
          icon="i-lucide-save"
    ...
    <main v-if="!isFetching && selectedRowData">
      <RidesReasonFormContentBasic :vertical="isMobile" :disable="isFetching || isPending" />
    </main>
  </div>
</template>
