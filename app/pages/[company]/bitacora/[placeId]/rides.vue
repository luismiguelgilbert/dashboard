...
const openNew = async () => {
  ...
  await navigateTo({ name: 'company-bitacora-placeId-rides-id', params: { id: newUniqueId }, query: { ...useRoute().query } })
};

const downloadFile = async () => {
  try {
    const response: Blob = await $fetch(`/api/${userCompany.value?.id}/bitacora/${userBitaPlace.value?.id}/rides-download`, {
      method: 'POST',
      body: {
        ...queryPayload.value,
        page: undefined,
...
onBeforeMount(() => {
  try {
    if (Object.keys(currentRoute.value.query).length) {
      const parsedQuery = bitacora_rides_query_schema.parse(currentRoute.value.query);
      push({ query: parsedQuery, replace: true });
      queryPayload.value = parsedQuery;
    } else {
      const parsedQuery = bitacora_rides_query_schema.parse(queryPayload.value);
      push({ query: parsedQuery, replace: true });
    }
  } catch {
    push({ query: {} });
...
<template>
  ...
  <USlideover
    ...
    :open="true"
    @update:open="navigateTo({ name: 'company-bitacora-placeId-rides', query: { ...useRoute().query } })">
    <template #content>
      <ClientOnly>
        <NuxtPage />
      </ClientOnly>
