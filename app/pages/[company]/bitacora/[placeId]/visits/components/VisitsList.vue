...
const fetcher = async (pageParam: number) => {
  const { data, error } = await useFetch(`/api/${userCompany.value?.id}/bitacora/${userBitaPlace.value?.id}/visits`, { method: 'post', headers, body: JSON.stringify({ ...queryPayload.value, page: pageParam }) })
  if (error.value) {
