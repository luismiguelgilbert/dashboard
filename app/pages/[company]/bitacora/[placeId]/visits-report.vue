  ...
  <UDashboardPanel id="bita-visits-report" class="lg:ml-5 border-l border-l-neutral-200 dark:border-l-neutral-800">
    ...
    <div class="overflow-y-auto">
      ...
      <div v-if="dateStart && dateEnd">
        <div>
          <div class="grid sm:flex items-start gap-10 p-3 border-b border-default">
            <div>
              <UUser
                size="2xl"
            ...
            <div class="grid">
              ...
              <span>
                Desde: <b class="text-muted">{{ `${DateTime.fromSQL(dateStart).setLocale('es-EC').toFormat('dd/MMMM/yyyy') }` }}</b>
                Hasta: <b class="text-muted">{{ `${DateTime.fromSQL(dateEnd).setLocale('es-EC').toFormat('dd/MMMM/yyyy') }` }}</b>
              </span>
            </div>
          </div>
          <UTable
