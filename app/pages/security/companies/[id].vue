...
<template>
  <div class="wrapper">
    <UDashboardNavbar
      ...
      <template #leading>
        <UButton
          ...
          @click="navigateTo({ name: 'security-companies', query: { ...useRoute().query } })" />
      </template>
      <template #right>
        <UButton
          icon="i-lucide-save"
    ...
    <main v-if="!isFetching && selectedRowData">
      <CompanyFormContentBasic :disable="isFetching || isPending" />
      <CompanyFormContentAvatar :disable="isFetching || isPending" />
    </main>
  </div>
</template>
