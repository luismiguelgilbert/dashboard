...
const emits = defineEmits<{'row-clicked': [row: sys_users]}>();
const { currentRoute } = useRouter();
const { dataList, dataListError, dataListStale, dataListFetching, dataUpdatedAt, fetchNextPage, hasNextPage } = useSecurityUsersQueries();
    ...
    <div
      ...
      <template v-for="page in dataList?.pages">
        <div
          ...
          <UUser
            class="p-3 pl-3 pr-6"
            :name="`${item.user_name} ${item.user_lastname}`"
