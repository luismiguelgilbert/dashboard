...
export const useBitacoraRidesQueries = () => {
  ...
  // DataList Actions
    ...
    queryFn: ({ pageParam }) => $fetch(`/api/${userCompany.value?.id}/bitacora/${userBitaPlace.value?.id}/rides`, { method: 'post', headers, body: JSON.stringify({ ...queryPayload.value, page: pageParam }) }),
    initialPageParam: 1,
  ...
  const { data: dataRecord, isFetching: dataRecordFetching } = useQuery({
    ...
    queryFn: () => $fetch(`/api/${userCompany.value?.id}/bitacora/${userBitaPlace.value?.id}/ride`, { method: 'post', headers, body: { id: currentRoute.value.params.id } }),
    staleTime: 1000 * 60 * 5, // 5 minutes
    enabled: Boolean(currentRoute.value.params.id), // Only run if an ID is provided
  });

  const postData = async (payload: bitacora_rides) => await $fetch(`/api/${userCompany.value?.id}/bitacora/${userBitaPlace.value?.id}/ride-upsert`, { method: 'POST', body: payload });
