...
export const useBitacoraCarsQueries = () => {
  ...
  // DataList Actions
    ...
    queryFn: ({ pageParam }) => $fetch(`/api/${userCompany.value?.id}/bitacora/cars`, { method: 'post', headers, body: JSON.stringify({ ...queryPayload.value, page: pageParam }) }),
    initialPageParam: 1,
  ...
  const postData = async (payload: bitacora_cars) => await $fetch(`/api/${userCompany.value?.id}/bitacora/car-upsert`, { method: 'POST', body: payload });
