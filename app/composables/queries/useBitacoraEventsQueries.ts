...
export const useBitacoraEventsQueries = () => {
  ...
  // DataList Actions
    ...
    queryFn: ({ pageParam }) => $fetch(`/api/${userCompany.value?.id}/bitacora/${userBitaPlace.value?.id}/events`, { method: 'post', headers, body: JSON.stringify({ ...queryPayload.value, page: pageParam }) }),
    initialPageParam: 1,
  ...
  const postData = async (payload: bitacora_events) => await $fetch(`/api/${userCompany.value?.id}/bitacora/${userBitaPlace.value?.id}/event-upsert`, { method: 'POST', body: payload });
