...
export const useBitacoraReasonsQueries = () => {
  ...
  // DataList Actions
    ...
    queryFn: ({ pageParam }) => $fetch(`/api/${userCompany.value?.id}/bitacora/reasons`, { method: 'post', headers, body: JSON.stringify({ ...queryPayload.value, page: pageParam }) }),
    initialPageParam: 1,
  ...
  const { data: dataRecord, isFetching: dataRecordFetching } = useQuery({
    ...
    queryFn: () => $fetch(`/api/${userCompany.value?.id}/bitacora/reason`, { method: 'post', headers, body: { id: currentRoute.value.params.id } }),
    staleTime: 1000 * 60 * 5, // 5 minutes
    enabled: Boolean(currentRoute.value.params.id), // Only run if an ID is provided
  });

  const postData = async (payload: bitacora_reasons) => await $fetch(`/api/${userCompany.value?.id}/bitacora/reason-upsert`, { method: 'POST', body: payload });
