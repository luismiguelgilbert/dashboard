...
export const useSecurityCompaniesQueries = () => {
  ...
  // DataList Actions
    ...
    queryFn: ({ pageParam }) => $fetch('/api/security/companies', { method: 'post', headers, body: JSON.stringify({ ...queryPayload.value, page: pageParam }) }),
    initialPageParam: 1,
  ...
  const postData = async (payload: sys_companies) => await $fetch('/api/security/company-upsert', { method: 'POST', body: payload });
