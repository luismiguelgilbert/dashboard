...
import { useActions, title, sort_options, filter_options } from './equipistas/components/config';
import { type type_ens_members } from '@/types/server/ens/ens_members';
import indexList from './equipistas/components/indexList.vue';

useHead({ title });
const { sessionData, handleUnauthorized } = useUserSession();
const { state, hasFilter } = useEnsEquipistas();
const router = useRouter();
const totalRows = computed<number>(() => data.value?.[0]?.row_count ?? 0 );
const isRightPanelOpen = computed<boolean>(() => router.currentRoute.value.name === 'ens-equipistas');
const { data, pending, refresh, error } = await useLazyFetch('/api/ens/equipistas', { method: 'post', body: state.value.filterPayload });
...
const setNewRoute = async (team: type_ens_members) => {
  state.value.selectedId = team.id!;
  router.push(`/ens/equipistas/equipista-${team.id}`);
};
</script>
