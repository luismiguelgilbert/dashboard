...
state.value.data = ens_servicios.cast({
  id: '',
  is_active: true,
  name_es: '',
});
...
const save = async () => {
  ...
  try {
    ...
    let response: type_ens_servicios_created = { id: '' };
    response = await $fetch('/api/ens/servicios/create', {
      method: 'post',
    ...
    await navigateTo(`/ens/servicios/servicio-${response.id}`);
  } catch (error) {
